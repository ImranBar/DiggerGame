...
void MainMenu::open(const vector <Texture> textures)
{
	...
	m_mapView = (make_unique<MenuBtn>(textures[2], Vector2f(m_playBtn->getPosition().x - 100, BUTTON_SIZE * 8), 35));
	m_mapView2 = (make_unique<MenuBtn>(textures[3], Vector2f(m_playBtn->getPosition().x + 100, BUTTON_SIZE * 8), 35));

	
	m_music = (make_unique<MenuBtn>(textures[4], Vector2f( m_playBtn->getPosition().x - 100 ,BUTTON_SIZE * 9 ), 35));
	m_mute = (make_unique<MenuBtn>(textures[5], Vector2f(m_playBtn->getPosition().x + 100, BUTTON_SIZE *9), 35));
